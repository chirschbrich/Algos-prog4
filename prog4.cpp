...
pair<int, vector<Applicant>> pickTeam(int budget, vector<vector<Applicant>>& alist) {
    ...
    vector<vector<float>> dp(positions + 1, vector<float>(budget + 1, 0)); // Store best benefit for max budget
    ...
    return make_pair(dp[positions][budget], team);
}
...
pair<int, vector<Applicant>> pickTeam_2(int budget, vector<vector<Applicant>>& alist) {
        ...
        for (int j = 0; j <= budget; j++) {
            ...
            for (int m = 0; m < alist[i - 1].size(); m++) {
                ...
                if (a1.cost <= j) { // Check if the cost of a1 is within the budget
                    float benefit = dp[i - 1][j - a1.cost] + a1.benefit; // Calculate the benefit of selecting a1
                    if (benefit > dp[i][j]) { // Check if the benefit is greater than the current best benefit
                        dp[i][j] = benefit;
                ...
                for (int k = m + 1; k < alist[i - 1].size(); k++) {
                    ...
                    if (a1.cost + a2.cost <= j) { 
                        float aBenefits = dp[i - 1][j - (a1.cost + a2.cost)] + a1.benefit + a2.benefit;
                        if (aBenefits > dp[i][j]) { // Check if the total benefits are greater than the current best benefit
                            dp[i][j] = aBenefits;
    ...
    return make_pair(dp[positions][budget], chosen[positions][budget]);
}